...
const callback = (filePath: string, basePath: string) => {
	...
	if (
		path.extname(filePath) === ".lua" ||
		path.extname(filePath) === ".zip" ||
		path.extname(filePath).startsWith(".part")
	) {
		const vfsPath = `build/vfs/${relPath}`;
		const vfsDir = path.dirname(vfsPath);
		if (!fs.existsSync(vfsDir)) {
...
const xml0 = fs.readFileSync(
	"../../vendor/PathOfBuilding/manifest.xml",
	"utf8",
);
const xml = xml0.replace(
	/<Version number="([0-9.]+)" \/>/,
...
fs.copyFileSync(
	"../../vendor/PathOfBuilding/changelog.txt",
	"build/vfs/changelog.txt",
);
