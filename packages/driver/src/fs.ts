...
class NodeEmscriptenEntryOps implements EmscriptenEntryOps {
    ...
    constructor(private _fs: EmscriptenFS, nodefs: any, FS: any, PATH: any, ERRNO_CODES: any) {
        this.nodefs = nodefs;
        this.FS = FS;
        this.PATH = PATH;
...
class NodeEmscriptenStreamOps implements EmscriptenStreamOps {
    ...
    constructor(private efs: EmscriptenFS, nodefs: any, FS: any, ERRNO_CODES: any) {
        this.nodefs = nodefs;
        this.FS = FS;
        this.ERRNO_CODES = ERRNO_CODES;
    }
...
export class NodeEmscriptenFS implements EmscriptenFS {
    ...
    public getMode(path: string): number {
        try {
            ...
            if (e.code) throw new this.fs.ErrnoError(this.errnoCodes[e.code]); else throw e;
        }
    }
