import type { ImageRepository } from "./image";
...
export class TextRasterizer {
	...
	static font(size: number, fontNum: number) {
		...
		switch (fontNum) {
			case 1:
				return `${size}px Liberation Sans`;
			case 2:
				return `${size}px Liberation Sans Bold`;
			case 0:
			default:
...
export class Renderer {
	...
	measureTextCursorIndex(
		...
		return this.textRasterizer.measureTextCursorIndex(
			size,
			font,
			text,
			cursorX,
