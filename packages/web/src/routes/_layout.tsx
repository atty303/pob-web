...
export type RootLayoutContext = {
  handleFrame: (render: boolean, time: number) => void;
  product: "poe1" | "poe2";
  version: string;
...
export default function RootLayout() {
  ...
  const context: RootLayoutContext = { handleFrame, product, version, setTitle };
...
function Footer(p: { frameTime: number }) {
  return (
    <>
      <footer className="footer flex items-center p-4 bg-neutral text-neutral-content">
        ...
        <div className="flex-none">
          <span className="badge badge-sm">Render: {p.frameTime.toFixed(1)}ms</span>
        </div>
        <div className="flex-none">
          <span className="link">
            <a href="https://github.com/atty303/pob-web/blob/main/CHANGELOG.md" target="_blank" rel="noreferrer">
              Version {APP_VERSION}
            </a>
          </span>
        </div>
